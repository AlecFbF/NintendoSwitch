...
function scene2()
{
  log("scene 2 IN");
   ...
   fbf.show(mc_introtxt,mc_introbg, mc_mariokart_logo_intro);

   TweenMax.to(mc_red,0.4,{ alpha:0, onComplete:fbf.displayNone,onCompleteParams:[mc_red] });

   //SMALL LOGOS FOR INTRO ANIMATE IN
   TweenMax.from(mc_switchlogo_intro2,1,{ delay:0, alpha:0 });
   TweenMax.from(mc_store_logo_introtxt,1,{ delay:0, alpha:0 });
   TweenMax.from(mc_mariokart_logo_intro,0.6,{ delay:.4, alpha:0, scale:0.2, ease: Power4.easeOut });
   //TweenMax.from(mc_header_txt,0.6,{ delay:.4, alpha:0, scale:0.2, ease: Power4.easeOut });

   
   //TXT 1 IN
   TweenMax.from(mc_introtxt_01,0.5,{ delay:0.6, ease: Power4.easeOut, x: -135 , alpha:0 });

   //TXT 1 OUT AND TXT 2 IN
   TweenMax.to(mc_introtxt_01,0.3,{ delay:5 , ease:Back.easeIn.config(1.7), alpha:0, x:430 , onComplete:fbf.displayNone,onCompleteParams:[mc_introtxt_01] });
   TweenMax.from(mc_introtxt_02,0.3,{ delay:5.3, ease: Power4.easeOut, x: -135 , alpha:0 });


   //TXT 2 OUT AND TXT 3 IN

   TweenMax.to(mc_introtxt_02,0.3,{ delay:7 , ease:Back.easeIn.config(1.7), alpha:0, x:430, onComplete:fbf.displayNone,onCompleteParams:[mc_introtxt_02] });
   TweenMax.from(mc_introtxt_03,0.3,{ delay:7.3, ease: Power4.easeOut, x: -135 , alpha:0 });

   //TXT 3 OUT
   TweenMax.to(mc_introtxt_03,0.3,{ delay:9 ,ease:Back.easeIn.config(1.7), alpha:0, x:430, onComplete:fbf.displayNone,onCompleteParams:[mc_introtxt_03] });

   TweenMax.delayedCall(9.3,scene3);

}
function scene3()
{
  log("scene 3 IN");
fbf.show(mc_product_seq,mc_bg,mc_store_logo,mc_mario_ribbon,mc_ribbon)
fbf.displayNone(mc_switch_intro);
    
    // GOODBYE RED INTRO
    TweenMax.to(mc_introbg,1,{ alpha:0, onComplete:fbf.displayNone,onCompleteParams:[mc_introbg] });
...
var _root = $('root');
	...
	var mc_introtxt = $('mc_introtxt');
		...
		var mc_mariokart_logo_intro = $('mc_mariokart_logo_intro');
